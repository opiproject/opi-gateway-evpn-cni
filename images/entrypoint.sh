...
EVPN_GW_BIN_FILE="/usr/bin/evpn-gw"

# Give help text for parameters.
usage()
{
    printf "This is an entrypoint script for EVPN GW CNI to overlay its\n"
...
for i in $CNI_BIN_DIR $EVPN_GW_BIN_FILE
do
  if [ ! -e "$i" ]; then
...
cp -f "$EVPN_GW_BIN_FILE" "$CNI_BIN_DIR"
